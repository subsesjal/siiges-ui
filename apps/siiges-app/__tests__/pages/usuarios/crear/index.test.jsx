...
jest.mock('next/router', () => ({
  ...
  default: { push: jest.fn(), back: jest.fn() },
}));
...
const NewUser = require('../../../../pages/usuarios/crear/index').default;

describe('Crear Usuario page (v2)', () => {
  it('renders without crashing', () => {
    const { container } = render(<NewUser />);
    expect(container).toBeTruthy();
  });

  it('renders v2 UserRoutePage with CREATE mode', () => {
    const { getByTestId } = render(<NewUser />);
    expect(getByTestId('user-route-page').textContent).toBe('CREATE');
  });
});
