...
export default function ConsultRevalidacion() {
  ...
  useEffect(() => {
    ...
    const fetchPaises = async () => {
      try {
        const paisesResponse = await getData({ endpoint: '/public/paises/' });
        if (paisesResponse.statusCode !== 200) {
          setError('Error al cargar los paises');
        }
        setPaises(paisesResponse.data);
      } catch (errorPaises) {
        console.error('¡Error al buscar Paises!:', errorPaises);
      }
    };
